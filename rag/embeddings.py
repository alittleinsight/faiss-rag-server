...
def is_gpu_usable(candidate_index: int) -> bool:
    test_code = (
        "import numpy\n"
        "import torch\n"
    ...
    print(f"Skipping GPU {candidate_index}: subprocess CUDA test error: {last_err}")
    return False
                ...
                if RAG_GPU_INDEX_RAW:
                    try:
                        ...
                        if 0 <= requested_index < device_count:
                            ...
                            print(f"Requested GPU index {requested_index} is out of range; "
                                  "trying all visible GPUs"
                            )
                    except ValueError:
                        print(f"Invalid RAG_GPU_INDEX value: '{RAG_GPU_INDEX_RAW}' trying all visible GPUs")
                        
                candidate_indices.extend(i for i in range(device_count) if i not in candidate_indices)

                for candidate_index in candidate_indices:
                    if is_gpu_usable(candidate_index):
